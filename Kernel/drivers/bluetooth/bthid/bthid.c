...
static int bthid_release(struct inode *inode, struct file *file)
{
    ...
    if (p_dev->hid) 
    {
        if (p_dev->hid->status == (HID_STAT_ADDED | HID_STAT_PARSED))//ansari_L&T_FROYO_CL534716
        {
...
{
    int ret;//ansari_L&T_FROYO_CL534716
    ...
    ret = hid_add_device(p_dev->hid);//ansari_L&T_FROYO_CL534716
